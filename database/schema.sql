...
CREATE TABLE gift_registries (
    ...
    group_id UUID NOT NULL REFERENCES groups(group_id) ON DELETE CASCADE,
    creator_id UUID NOT NULL REFERENCES group_members(group_member_id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    sharing_type VARCHAR(20) NOT NULL CHECK (sharing_type IN ('public', 'passcode', 'group_only')),
    passcode VARCHAR(6),
    web_token VARCHAR(32) UNIQUE NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_gift_registries_group ON gift_registries(group_id);
CREATE INDEX idx_gift_registries_creator ON gift_registries(creator_id);
CREATE INDEX idx_gift_registries_web_token ON gift_registries(web_token);

CREATE TABLE gift_items (
    item_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    registry_id UUID NOT NULL REFERENCES gift_registries(registry_id) ON DELETE CASCADE,
    title VARCHAR(255) NOT NULL,
    link TEXT,
    photo_url TEXT,
    cost DECIMAL(10, 2),
...
CREATE INDEX idx_gift_items_registry ON gift_items(registry_id, display_order);
