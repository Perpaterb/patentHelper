...
/**
 * VideoCallsScreen component
 *
 * @param {VideoCallsScreenProps} props
 * @returns {JSX.Element}
 */
export default function VideoCallsScreen({ navigation, route }) {
  ...
  // Reload video calls when screen comes into focus
  useFocusEffect(
    React.useCallback(() => {
      loadVideoCalls();
    }, [groupId])
  ...
  /**
   * Load video calls from API
   */
  const loadVideoCalls = async () => {
    try {
      setError(null);
      const response = await api.get(`/groups/${groupId}/video-calls`);
      setVideoCalls(response.data.videoCalls || []);
    } catch (err) {
      console.error('Load video calls error:', err);

      if (err.isAuthError) {
        console.log('[VideoCalls] Auth error detected - user will be logged out');
        return;
      }

      setError(err.response?.data?.message || 'Failed to load video calls');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMakeCall = () => {
    navigation.navigate('InitiateVideoCall', { groupId, members });
  };
  ...
  const handleCallPress = (call) => {
    navigation.navigate('VideoCallDetails', {
      groupId,
      callId: call.callId,
      call
    });
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptySubtext}>
        {canMakeCalls
          ? 'Start a video call to connect face-to-face with your group'
          : 'No video calls have been made in this group yet'}
      </Text>
    </View>
  );
