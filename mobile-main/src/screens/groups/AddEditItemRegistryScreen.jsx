/**
 * Add/Edit Item Registry Screen
 *
 * Allows users to create a new item registry or edit an existing one.
...
/**
 * @typedef {Object} AddEditItemRegistryScreenProps
 * @property {Object} navigation - React Navigation navigation object
 * @property {Object} route - React Navigation route object
 */

/**
 * AddEditItemRegistryScreen component
 *
 * @param {AddEditItemRegistryScreenProps} props
 * @returns {JSX.Element}
 */
export default function AddEditItemRegistryScreen({ navigation, route }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await api.post(`/groups/${groupId}/item-registries`, {
        name: name.trim(),
      ...
      if (err.isAuthError) {
        console.log('[AddEditItemRegistry] Auth error detected - user will be logged out');
        return;
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await api.put(`/groups/${groupId}/item-registries/${registryId}`, {
        name: name.trim(),
      });
      ...
      <ScrollView style={styles.scrollView}>
        <View style={styles.content}>
          <Title style={styles.title}>
            {isEditMode ? 'Edit Item Registry' : 'Create Item Registry'}
          </Title>
        <Text style={styles.subtitle}>
          {isEditMode
            ? 'Update the name of your item registry.'
            : 'Create a new item registry to track shared items and borrowing.'}
        </Text>
        ...
        <TextInput
          ...
          mode="outlined"
          style={styles.input}
          placeholder="e.g., Household Items, Tools, Books"
          disabled={loading}
          maxLength={255}
        />
