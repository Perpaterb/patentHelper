...
/**
 * Get LiveKit connection info for a video call
...
async function startLivekitRecording(req, res) {
  try {
    ...
    const call = await prisma.videoCall.findUnique({ where: { callId } });
    if (!call || call.groupId !== groupId) {
      return res.status(404).json({ success: false, message: 'Call not found' });
    }
    ...
    const roomName = `video-call-${callId}`;
    const outputPath = `video-calls/${groupId}/${callId}`;

    try {
      const egress = await livekitService.startRoomRecording(roomName, 'video', outputPath);
      ...
      console.error('Start video recording egress error:', egressError);
      return res.status(500).json({
        success: false,
    ...
    console.error('Start LiveKit video recording error:', error);
  ...
  try {
    ...
    try {
      ...
      const recordingUrl = s3Bucket
        ? `https://${s3Bucket}.s3.amazonaws.com/recordings/video-calls/${groupId}/${callId}.mp4`
        : `/recordings/video-calls/${groupId}/${callId}.mp4`;
      ...
      console.error('Stop video recording egress error:', egressError);
    ...
    console.error('Stop LiveKit video recording error:', error);
